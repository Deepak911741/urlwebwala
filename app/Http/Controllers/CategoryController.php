...
class CategoryController extends MY_Controller
{
    public function __construct(){
        ...
        $this->tableName = config('constants.CATEGORY_TABLE');
        $this->moduleName = trans('messages.category');
        $this->folderName = config('constants.ADMIN_FOLDER') . 'category/'; 
    }
    ...
    public function add(Request $request){
        ...
        $formValidation = [];
        $formValidation['category_name'] = ['required'];
        
        $validator = Validator::make($request->all(), $formValidation, [
            'category_name.required' => trans("messages.required-enter-field-validation" , [ "fieldName" => trans("messages.category") ]),
        ]);
        ...
        $recordData['v_category_name'] = (checkNotEmptyString($request->input('category_name')) ? trim($request->input('category_name')) : null);
        ...
        try{
            if($recordId > 0){
                ...
                $this->crudModel->updateTableData(config('constants.CATEGORY_TABLE') , $recordData , ['i_id' => $recordId]);
            } else {
                $this->crudModel->insertTableData(config('constants.CATEGORY_TABLE') , $recordData);
            }
			$result = true;
		}catch(\Exception $e){
			$result = false;
    ...
    public function list(Request $request) {
        ...
        if(!empty($recordDetails)){
            ...
            foreach($recordDetails as $key => $recordDetail){
                ...
                $rowData['title'] = (checkNotEmptyString($recordDetail->v_category_name) ? $recordDetail->v_category_name : '' );
    ...
    private function commonFilterData($request = null){
		...
		if(checkNotEmptyString($request->post('search_by')) ){
			...
			$likeData = [ 'value' => $searchValue, 'columnName' => ['v_category_name'] ];
		}
		...
		if(!empty($recordId)){
			...
			if( $recordId > 0 ){
				...
				if(!empty($recordInfo)){
					...
					$data['category_name'] = $recordInfo->v_category_name;
